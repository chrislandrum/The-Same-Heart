...
function register_share() {
	...
	if (!document.getElementById('terms').checked){
		...
		if(email == '') {
			...
			jQuery.ajax({
				type: "POST",
				url: 'http://cunjo.com/wp-content/themes/nubpage/inc/ajaxify.php',
				data: data,
...
function get_barform(stepz) {
	...
	var data = {action:'buildbar',step:stepz,ltype:ltypez};
		...
		var data = {action:'buildbar',step:stepz,socials:socials2,message:message2,linx:linx2,iconx:iconx2,theme:theme2,iconz:iconz2,width:width2,position:position2,showat:showat2};
	}
	jQuery.post(blogVars.ajaxUrl, data, function(response) {
		if(response == 'no') {
			jQuery('.register-mess').html('<div class="widget-alert">There was an error, please try again later.</div>');
...
function get_lineform(stepz) {
	...
	if(stepz == 2) {
		...
		var data = {action:'buildline',step:stepz,socials:socials2,theme:theme2,iconz:iconz2,position:position2,showing:showing2};
	}
	jQuery.post(blogVars.ajaxUrl, data, function(response) {
		if(response == 'no') {
			jQuery('.register-mess').html('<div class="widget-alert">There was an error, please try again later.</div>');
