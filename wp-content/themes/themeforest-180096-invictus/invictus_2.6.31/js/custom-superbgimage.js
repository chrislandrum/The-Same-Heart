...
jQuery(document).ready(function($) {
	...
	if( jQuery('html').hasClass('touch') ){
		isTouch = true,
		click_event = 'touchend'; // change click event for play button to touchstart
	}
...
/*-----------------------------------------------------------------------------------*/
	...
	if(isTouch){
		...
		jQuery("#superbgimage, #scanlines").swipe("destroy");
		jQuery("#superbgimage, #scanlines").addFullsizeTouchswipe();

		// add the drag scroll function
		var o = jQuery("#thumbnailContainer").overscroll({
			cancelOn: '.no-drag',
