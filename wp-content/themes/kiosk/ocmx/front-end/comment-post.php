<?php function ocmx_comment_post()
	{
		...
		if ( get_option('require_name_email') && !$user->ID ) {
			if ( 6 > strlen($comment_author_email) || '' == $comment_author )
				die( __('<p>Error: please fill the required fields (name, email).</p>','ocmx') );
			elseif ( !is_email($comment_author_email))
				die( __('<p>Error: please enter a valid email address.</p>','ocmx') );
		}
		
		if ( '' == $comment_content )
			die( __('<p>Error: please type a comment.</p>','ocmx') );
